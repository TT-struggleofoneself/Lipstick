...
$sql = "select * from ".tablename('mzhk_sun_cardorder')." ".$where;


if($_GPC['op']=='exportorder'){
    $select_sql =$sql." order by id asc ";
...
$total=pdo_fetchcolumn("select count(*) as wname from ".tablename('mzhk_sun_cardorder')." ".$where,$data);
...
if($_GPC['op']=='change'){
    ...
    $order = pdo_get('mzhk_sun_cardorder',array('uniacid'=>$uniacid,'id'=>$id),array("money","bid","gid","ordernum","status"));
    if($status==2){
        if($order["status"]==2){
