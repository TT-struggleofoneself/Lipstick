...
if($_GPC['op']=='send'){
    ...
    $order = pdo_get('mzhk_sun_order',array('uniacid'=>$uniacid,'oid'=>$id),array("money","bid","gid","orderNum","status"));
    if($order["status"]==5){
    ...
    if($res){
        ...
        $data["mcd_type"] = 1;
        $data["mcd_memo"] = "普通订单-订单id：".$id.";订单号：".$order["orderNum"]."；";
        $data["addtime"] = time();
...
if($_GPC['op']=='refund'){
    ...
    $order = pdo_get('mzhk_sun_order',array('uniacid'=>$uniacid,'oid'=>$id),array("money","out_trade_no","out_refund_no","gid","paytype","openid"));
...
$sql = "select * from ".tablename('mzhk_sun_order')." ".$where;
...
$total=pdo_fetchcolumn("select count(*) as wname from ".tablename('mzhk_sun_order')." ".$where,$data);
