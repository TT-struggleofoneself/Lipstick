...
define('CLOUD_GATEWAY_URL', HTTP_X_FOR . '#');
define('CLOUD_GATEWAY_URL_NORMAL', HTTP_X_FOR . '#');
...
function cloud_build() {
    ...
    $dat = cloud_request(HTTP_X_FOR . '#', $pars);
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request(HTTP_X_FOR . '#', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request(HTTP_X_FOR . '#', $pars);
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request(HTTP_X_FOR . '#', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
